...
import { FoodCostHistory, Recipe, RecipeIngredient, RecipeInstruction } from '@ckm/db';
...
export class RecipeService {
  ...
  async createRecipe(data: Prisma.RecipeCreateInput): Promise<Recipe> {
    try {
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        if (error.code === 'P2002') {
          throw new BadRequestException('Recipe with the same name already exists');
        }
      }
      throw error;
    }
  ...
  // async updateRecipe(id: number, input: Prisma.RecipeUpdateInput): Promise<Recipe> {
  //   try {
   ...
   */
  async removeIngredientFromRecipe(recipeId: number, id: number): Promise<RecipeIngredient> {
    ...
    try {
      ...
      if (!recipeIngredient || recipeIngredient.recipeId !== recipeId) {
        throw new NotFoundException(`Ingredient with ID ${id} not found in recipe ${recipeId}`);
      }
   ...
   */
  async getRecipeIngredient(recipeId: number, id: number): Promise<RecipeIngredient> {
    ...
    try {
      ...
      if (!ingredient || ingredient.recipeId !== recipeId) {
        throw new NotFoundException(`Ingredient with ID ${id} not found for recipe ${recipeId}`);
      }
      return ingredient;
    } catch (error) {
   ...
   */
  async removeInstructionFromRecipe(recipeId: number, id: number): Promise<RecipeInstruction> {
    ...
    try {
      ...
      if (!instruction || instruction.recipeId !== recipeId) {
        throw new NotFoundException(`Instruction with ID ${id} not found for recipe ${recipeId}`);
      }
  ...
  async updateInstructionInRecipe(
    ...
    try {
      ...
      if (data.instruction) {
        data.instruction = await this.encryptionService.encrypt(data.instruction as string);
      }
   ...
   */
  async getRecipeInstruction(recipeId: number, id: number): Promise<RecipeInstruction> {
    ...
    try {
      ...
      const decryptedInstructions = await Promise.all(
        instructions.map(async instruction => ({
          ...instruction,
          instruction: await this.encryptionService.decrypt(instruction.instruction),
        })),
      );
   ...
   */
  async calculateRecipePrice(recipeId: number, profitMargin: number = 0.3): Promise<number> {
   ...
   */
  async getRecipePrice(recipeId: number, profitMargin?: number): Promise<number> {
