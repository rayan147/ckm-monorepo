...
export class PrepBoardController {
  ...
  async createPrepBoard() {
    return tsRestHandler(
      contract.prepBoard.createPrepBoard,
      async ({ body }) => {
  ...
  async getPrepBoards() {
    return tsRestHandler(
      contract.prepBoard.getPrepBoards,
      async ({ query }) => {
  ...
  async getPrepBoard() {
    return tsRestHandler(
      contract.prepBoard.getPrepBoard,
      async ({ params }) => {
        this.logger.log(
          `Received request to get prep board with ID ${params.id}`,
        );
  ...
  async updatePrepBoard() {
    return tsRestHandler(
      contract.prepBoard.updatePrepBoard,
      async ({ params, body }) => {
        this.logger.log(
          `Received request to update prep board with ID ${params.id}`,
        );
        const prepBoard = await this.prepBoardService.updatePrepBoard(
          params.id,
          body,
        );
        return { status: 200, body: prepBoard };
  ...
  async deletePrepBoard() {
    return tsRestHandler(
      contract.prepBoard.deletePrepBoard,
      async ({ params }) => {
        this.logger.log(
          `Received request to delete prep board with ID ${params.id}`,
        );
        const prepBoard = await this.prepBoardService.deletePrepBoard(
          params.id,
        );
        return {
          status: 200,
