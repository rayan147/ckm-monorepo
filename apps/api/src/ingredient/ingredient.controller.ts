...
export class IngredientController {
  ...
  async createIngredient() {
    return tsRestHandler(
      contract.ingredient.createIngredient,
      async ({ body }) => {
  ...
  async getIngredients() {
    return tsRestHandler(
      contract.ingredient.getIngredients,
      async ({ query }) => {
  ...
  async getIngredient() {
    return tsRestHandler(
      contract.ingredient.getIngredient,
      async ({ params }) => {
        this.logger.log(
          `Received request to get ingredient with ID ${params.id}`,
        );
        const ingredient = await this.ingredientService.getIngredient(
          params.id,
        );
  ...
  async updateIngredient() {
    return tsRestHandler(
      contract.ingredient.updateIngredient,
      async ({ params, body }) => {
        this.logger.log(
          `Received request to update ingredient with ID ${params.id}`,
        );
  ...
  async deleteIngredient() {
    return tsRestHandler(
      contract.ingredient.deleteIngredient,
      async ({ params }) => {
        this.logger.log(
          `Received request to delete ingredient with ID ${params.id}`,
        );
        const ingredient = await this.ingredientService.deleteIngredient(
          params.id,
        );
        return {
          status: 200,
