<script lang="ts">
	...
	let prepBoards = $state([]);
	let currentView = $state('list');
	let currentPrepBoard = $state(null);
	...
	async function addPrepBoard() {
		currentPrepBoard = { name: '', status: 'PENDING', recipes: [], prepItems: [] };
		currentView = 'create';
	}

	async function editPrepBoard(board) {
		currentPrepBoard = { ...board };
		currentView = 'edit';
	}

	async function deletePrepBoard(id) {
	...
	async function savePrepBoard() {
		try {
			if (currentView === 'create') {
				await api.prepBoard.createPrepBoard({ body: currentPrepBoard });
			} else {
				await api.prepBoard.updatePrepBoard({
					params: { id: currentPrepBoard.id },
					body: currentPrepBoard
				});
			}
			await fetchPrepBoards();
			currentView = 'list';
