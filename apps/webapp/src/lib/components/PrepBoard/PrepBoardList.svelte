<script lang="ts">
	...
	import type { PrepBoard, PrepItem, Recipe } from '../../../../../server/prisma/generated/zod';
	import { api } from '@ckm/lib-api';

	interface PrepBoardWithRelations extends PrepBoard {
		prepItems: PrepItem[];
		recipes: Recipe[];
	}

	let prepBoards: PrepBoardWithRelations[] = $state([]);
	let currentView = $state('list');
	let currentPrepBoard: PrepBoardWithRelations | null = $state(null);
	let loading = $state(true);
	let error: string | null = $state(null);

	onMount(async () => {
		await fetchPrepBoards();
	});

	async function fetchPrepBoards() {
		try {
			loading = true;
			const response = await api.prepBoard.getPrepBoards({});
			if (response.status === 200) {
				prepBoards = response.body;
			} else {
	...
	function editPrepBoard(board: PrepBoardWithRelations) {
	...
	async function deletePrepBoard(id: number) {
		if (confirm('Are you sure you want to delete this prep board?')) {
			try {
				await api.prepBoard.deletePrepBoard({ params: { id } });
				await fetchPrepBoards();
			} catch (err) {
				error = 'Failed to delete prep board';
			}
		}
	}
	...
	async function savePrepBoard() {
		try {
			if (!currentPrepBoard) return;

			const prepBoardData = {
				name: currentPrepBoard.name,
				status: currentPrepBoard.status,
				recipes: { connect: currentPrepBoard.recipes.map((recipe) => ({ id: recipe.id })) },
				prepItems: { connect: currentPrepBoard.prepItems.map((item) => ({ id: item.id })) }
			};

			if (currentView === 'create') {
				await api.prepBoard.createPrepBoard({ body: prepBoardData });
			} else {
				await api.prepBoard.updatePrepBoard({
					params: { id: currentPrepBoard.id },
					body: prepBoardData
				});
			}
			await fetchPrepBoards();
			...
			error = 'Failed to save prep board';
		}
	}

	function viewPrepBoard(board: PrepBoardWithRelations) {
		currentPrepBoard = { ...board };
		currentView = 'view';
	}
...
<div class="px-4 py-8 mx-auto max-w-7xl sm:px-6 lg:px-8">
	{#if currentView === 'list'}
		<div class="flex items-center justify-between mb-8">
			<h1 class="text-3xl font-semibold text-gray-900">Prep Boards</h1>
			<button
		...
		{:else}
			<div class="grid grid-cols-1 gap-6 mt-8 sm:grid-cols-2 lg:grid-cols-3">
				{#each prepBoards as board (board.id)}
						...
						<div class="px-4 py-5 sm:p-6">
							<div class="flex items-center justify-between mb-4">
								<div class="flex items-center">
									<SvelteComponent
										class="w-8 h-8 mr-3 text-gray-400"
									/>
	...
	{:else if currentView === 'view' && currentPrepBoard}
										...
										<li class="flex items-center justify-between py-3 pl-3 pr-4 text-sm">
											<div class="flex items-center flex-1 w-0">
												<span class="flex-1 w-0 ml-2 truncate"
													>{item.recipe?.name || 'Unknown Recipe'} (Qty: {item.quantity})</span
												>
											</div>
